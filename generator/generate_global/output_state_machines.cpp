...
void output_state_machines(StateMachineArray &state_machine_array, MemoryArena &arena) {
	{
		for (int i = 0; i < array_count(state_machine_array); ++i) {
			StateMachine &state_machine = state_machine_array[i];

			String name_underscore = make_underscore_case(state_machine.name, arena);
			String output_filepath = make_filepath(arena, _folder_root, _folder_generated_code, name_underscore, MAKE_STRING("_state_machine.generated.h"));
			MAKE_OUTPUT_FILE_WITH_HEADER(output, *output_filepath);

			for (int j = 0; j < array_count(state_machine.states); ++j) {
...
fprintf(output, "\n");
			...
			String *short_names = (String *)allocate_memory(arena, array_count(state_machine.states)*sizeof(String));
			String *short_names_underscore = (String *)allocate_memory(arena, array_count(state_machine.states)*sizeof(String));
...
fprintf(output, "struct %sStateMachine {\n", *state_machine.name);
			for (int j = 0; j < array_count(state_machine.functions); ++j) {
...
fprintf(output, "};\n");
	...
	}
	{
		for (int i = 0; i < array_count(state_machine_array); ++i) {
			...
			String name_underscore = make_underscore_case(state_machine.name, arena);
			String output_filepath = make_filepath(arena, _folder_root, _folder_generated_code, name_underscore, MAKE_STRING("_state_machine.generated.cpp"));
			MAKE_OUTPUT_FILE_WITH_HEADER(output, *output_filepath);
...
fprintf(output, "	switch (active_type) {\n");
				for (int k = 0; k < array_count(state_machine.states); ++k) {
