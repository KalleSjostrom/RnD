...
void output_network_config(ComponentArray &component_array, RPCArray &rpc_array, NetworkTypeArray &network_type_array, GameObjectArray &go_array, MemoryArena &arena) {
	{
...
fprintf(output,  "	vector3 = { type = \"vector3\", min = -10000, max = 10000, bits = 31 }\n");
...
fprintf(output,  "	entity = { type = \"int\", min = 0, max = 65535 }\n");
fprintf(output,  "	game_object_id = { type = \"int\", min = 0, max = 65535 }\n");
fprintf(output,  "	peer_id = { type = \"int64\" }\n");
fprintf(output,  "	ticket_id = { type = \"int64\", min = 0, bits = 64 }\n");
...
fprintf(output,  "	// Generated component types\n");
			for (int i = 0; i < array_count(component_array); ++i) {
				...
				if (HAS_SUB_COMP(SubCompType_Network)) {
					...
					for (int k = 0; k < array_count(network.member_array); k++) {
						output_member_type(output, component.stem, network.member_array[k], arena);
					}
				}
			}
fprintf(output,  "\n");
...
fprintf(output,  "messages = {\n");
			for (int i = 0; i < array_count(rpc_array); ++i) {
				RPCFunction &rpc = rpc_array[i];
				Function &function = rpc.function;
fprintf(output,  "	%s = {\n", *function.name);
				if (array_count(function.parameters) > 1) {
fprintf(output,  "		args = [\n");
					for (unsigned j = 1; j < array_count(function.parameters); ++j) {
...
fprintf(output,  "		fields = [\n");
				...
				for (int j = 0; j < array_count(settings->component_settings); ++j) {
					...
					SubCompStruct *sub_comps = component.sub_comps;
					if (HAS_SUB_COMP(SubCompType_Network) && array_count(sub_comps[SubCompType_Network].member_array) > 0) {
						SubCompStruct &network = sub_comps[SubCompType_Network];
fprintf(output,  "			// %s\n", *component.stem);
						for (int k = 0; k < array_count(network.member_array); k++) {
							Member &member = network.member_array[k];
							if (member.exported_line_counter > 0) {
