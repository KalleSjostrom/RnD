...
fprintf(output, "				event_delegate->trigger_%s(", *function.name);
		...
		for (unsigned j = optional_index >= 0 ? 1 : 0; j < array_count(function.parameters); ++j) {
...
void output_flow(FlowArray &flow_array, MemoryArena &arena) {
	{ // Prepend the function name to the mapped names
		for (int i = 0; i < array_count(flow_array); ++i) {
			FlowFunction &flow = flow_array[i];
			Function &function = flow.function;
			for (int j = 0; j < flow.function_map.count; ++j) {
...
fprintf(output, "	{\n");
			FlowFunction &flow = flow_array[i];
			Function &function = flow.function;
			bool ignore_first_parameter = flow.function_map.count > 0; // If we have a function map, ignore the first parameter. This is reserved as the index of the input event.
			unsigned start_index = ignore_first_parameter ? 1 : 0;
fprintf(output, "		name = \"%s\"\n", *make_pretty_flow_name(function.name, arena));
fprintf(output, "		args = {\n");
			for (unsigned j = start_index; j < array_count(function.parameters); ++j) {
				Parameter &parameter = function.parameters[j];
fprintf(output, "			%s = \"%s\"\n", *parameter.name, convert_flow_type(parameter.type_id));
			}
fprintf(output, "		}\n");

			if (array_count(flow.return_list) > 0) {
fprintf(output, "		returns = {\n");
				for (int j = 0; j < array_count(flow.return_list); ++j) {
...
fprintf(output, "	// Parameter ids: \n");
			for (int j = 0; j < array_count(function.parameters); ++j) {
				Parameter &parameter = function.parameters[j];
...
fprintf(output, "					for (unsigned j = 0; j < ARRAY_COUNT(result.%s) && !end_found; j++) {\n", *parameter.name);
fprintf(output, "						result.%s[j] = *cursor;\n", *parameter.name);
