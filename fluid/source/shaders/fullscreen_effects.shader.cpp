...
static const char *fragment = GLSL(
	...
	void main() {
		...
		vec4 shadow_vert_color = texture(shadow, rotate_uv(uv, -0.0f).xy*2);
		vec4 shadow_hori_color = texture(shadow, rotate_uv(uv, -0.0f).yx*2);
		...
		float a1 = 1 - shadow_vert_color.g;

		float shadow = shadow_vert_color.r + shadow_hori_color.r;
		float a2 = (1-shadow) * (1-scene_color.a) + scene_color.a;
