...
pushd "%OUT_PATH%"
	rem echo    Trying to remove pdbs, dlls and still in the fluid folder
...
set DISABLE_WARNINGS=%DISABLE_WARNINGS% /wd4060 /wd4068 /wd4201 /wd4127 /wd4191 /wd4505 /wd4100

set CC=cl.exe
set FLAGS=-nologo -fp:fast -Gm- -GR- -EHa- -FC -Z7 -GF -WL -Wall /arch:AVX
...
pushd "%OUT_PATH%"
	...
	if %INSTANCE_RUNNING%==1 (
		...
		rem If so, we need to let the engine process know there's a new fluid dll to load
