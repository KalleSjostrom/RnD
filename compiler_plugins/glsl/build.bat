...
echo Building glsl compiler plugin
set NAME=plugin
...
%CC% %FLAGS% %DEFINES% %INCLUDES% %LIBS% %NAME%.cpp %DISABLE_WARNINGS% -Fe%OUT_PATH%/%NAME%.exe -Fd%OUT_PATH%/%NAME%.pdb %LIBS% -LD /link -incremental:no -opt:ref -PDB:"%OUT_PATH%\%NAME%.pdb" -OUT:"%OUT_PATH%\%NAME%.dll"