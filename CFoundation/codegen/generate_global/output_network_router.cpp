bool conversion_write(FILE *output, NetworkConversionArray &network_conversion_array, unsigned type_id, String &name) {
	for (int i = 0; i < network_conversion_array.count; ++i) {
		NetworkConversion &network_conversion = network_conversion_array.entries[i];
...
NetworkType *find_network_type(NetworkTypeArray &network_type_array, unsigned name_id) {
	for (int i = 0; i < network_type_array.count; ++i) {
		if (network_type_array.entries[i].name_id == name_id) {
			return network_type_array.entries + i;
		}
	}
	return 0;
}
...
bool is_rpc_dynamic(Function &rpc, NetworkTypeArray &network_type_array) {
	...
	for (unsigned i = 1; i < rpc.parameter_count; ++i) {
...
fprintf(output, "	// RPC names:\n");
		for (unsigned i = 0; i < rpc_array.count; ++i) {
			Function &rpc = rpc_array.entries[i].function;
...
fprintf(output, "		void %s(PeerId receiving_peer", *rpc.name);
			for (unsigned j = 1; j < rpc.parameter_count; ++j) {
...
fprintf(output, "		}\n");
fprintf(output, "		void game_session_disconnect(PeerId sending_peer) { LOG_INFO(\"Network\", \"game_session_disconnect NOT IMPLEMENTED! (sender=%%llx)\", sending_peer); }\n");
fprintf(output, "\n");
...
fprintf(output, "			EntityRef entity = components::manager->get_entity(go_id);\n");
fprintf(output, "\n");
...
fprintf(output, "				unsigned num_peers = _GameSession.num_peers(game_session);\n");
fprintf(output, "				for (unsigned i = 0; i < num_peers; i++) {\n");
fprintf(output, "					PeerId receiver = _GameSession.peer(game_session, i);\n");
fprintf(output, "					_Network.send_rpc(id, receiver, parameters, num_parameters);\n");
fprintf(output, "				}\n");
fprintf(output, "			} else if (receiving_peer == RPC_OTHERS) {\n");
fprintf(output, "				GameSessionPtr game_session = _Network.game_session(); \n");
fprintf(output, "				unsigned num_peers = _GameSession.num_peers(game_session);\n");
fprintf(output, "				PeerId me = _Network.peer_self();\n");
fprintf(output, "				for (unsigned i = 0; i < num_peers; i++) {\n");
fprintf(output, "					PeerId receiver = _GameSession.peer(game_session, i);\n");
fprintf(output, "					if (receiver != me)\n");
fprintf(output, "						_Network.send_rpc(id, receiver, parameters, num_parameters);\n");
fprintf(output, "				}\n");
