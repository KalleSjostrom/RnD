...
inline Quaternion quaternion(const Matrix4x4 &mat) {
	...
	const Vector3 _scale = scale(mat);
	const float *s = reinterpret_cast<const float *>(&_scale);

	// If _scale is small return an identity quaternion
	const float eps = 1e-5;
	if ((_scale.x > -eps && _scale.x < eps) || (_scale.y > -eps && _scale.y < eps) || (_scale.z > -eps && _scale.z < eps))
