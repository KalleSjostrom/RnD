...
__forceinline float distance_xy_squared(const Vector3 &v0, const Vector3 &v1) {
...
__forceinline float angle_z(const Vector3 &a, const Vector3 &b) {
	float d = dot(a, b);
	return math::sign(cross(a, b).z) * acos(math::clamp(d, -1.0f, 1.0f));
}
