...
unsigned __stdcall _thread_start(void *params) {
	...
	// We might never get here if the fiber we switch to never switches back to us!
	exit_thread(); // Cleanup and shutdown
...
int main(int argc, char const *argv[]) {
	...
	MemoryArena arena;
	setup_arena(&arena, 1024 * 1024, default_push_params()); // 1 MB

	ThreadContext *thread_contexts = PUSH_STRUCTS(&arena, thread_count, ThreadContext, default_push_params());
	ThreadType *threads = PUSH_STRUCTS(&arena, thread_count, ThreadType, default_push_params());
	...
	int index = _get_free_fiber_index(scheduler);
	return 0;
}
