...
/// Wrap os threading
	...
	typedef unsigned(__stdcall *ThreadStartRoutine)(void *arg);
	#define THREAD_FUNC_RETURN_TYPE unsigned

	inline bool create_thread(ThreadStartRoutine start_routine, void *arg, ThreadType *out_thread, unsigned stack_size, int thread_affinity) {
		HANDLE handle = (HANDLE) _beginthreadex(0, stack_size, start_routine, arg, CREATE_SUSPENDED, 0);
		...
		if (thread_affinity) {
			SetThreadAffinityMask(handle, thread_affinity);
		}
	...
	inline bool create_thread(ThreadStartRoutine start_routine, void *arg, ThreadType *out_thread, unsigned stack_size = (unsigned)-1) {
		pthread_attr_t thread_attr;
		pthread_attr_init(&thread_attr);

		// Set stack size
		if (stack_size != (unsigned)-1) {
