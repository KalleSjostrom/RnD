...
namespace parser {
	...
	static THREAD_LOCAL ParserContext *global_last_parser_context;
	...
	inline void set_parser_context(ParserContext &context, Tokenizer *tokenizer, char *filepath) {
	...
	inline Tokenizer make_tokenizer(char *at, size_t debug_size, u32 flags) {
		Tokenizer t = {};
		t.flags = flags;
		t.at = at;
		t.debug_start = at;
	...
	float parse_number(Tokenizer *tok, int sign, int integer, bool do_fraction, bool do_sientific = false) {
		if (do_fraction) {
			...
			float value = sign * (integer + (fraction/(float)divisor));
			if (tok->at[0] == 'e') {
				tok->at++;
				divisor = 1;
				int exponent = 0;
...
#define ASSERT_TOKEN(tok, token, expected_token) { \
	{ \
		...
		parser::global_last_parser_context->tokenizer = &tok; \
