...
enum {
  RE_UNUSED, RE_DOT, RE_BEGIN, RE_END, RE_QUESTIONMARK, RE_STAR, RE_PLUS, RE_CHAR, RE_CHAR_CLASS,
  RE_INV_CHAR_CLASS, RE_DIGIT, RE_NOT_DIGIT, RE_ALPHA, RE_NOT_ALPHA, RE_WHITESPACE, RE_NOT_WHITESPACE, /* BRANCH */
};
      ...
      case '\\':
      {
        if (pattern[i+1] != '\0')
        {
          ...
          switch (pattern[i])
          {
            ...
            case 'S': {    re_compiled[j].type = RE_NOT_WHITESPACE;   }
            break;
...
static int matchone(regex_t p, char c)
{
  switch (p.type)
  {
    ...
    case RE_CHAR_CLASS:     return  matchcharclass(c, (const char*)p.ccl);
    case RE_INV_CHAR_CLASS: return !matchcharclass(c, (const char*)p.ccl);
