...
pushd "%OUT_PATH%"
	rem echo    Trying to remove pdbs, dlls and still in the raytracer folder
...
if %INSTANCE_RUNNING%==1 (
	...
	set PLUGIN_NAME=genome_%random%
) else (
	set PLUGIN_NAME=raytracer
)
...
set INCLUDES=-I ../
set ENTRY_POINT=source/raytracer
...
pushd "%OUT_PATH%"
	...
	if %INSTANCE_RUNNING%==1 (
		...
		rem If so, we need to let the engine process know there's a new raytracer dll to load
